# 129. **보너스: 업로드한 이미지를 클라우드에 저장하기(AWS S3)**
...
새로운 `src`값은 버킷 객체로 연결되는 S3 URL (즉, 테스트 목적으로 클릭한 URL으로, 이미지 파일명이 끝에 없어야 합니다)을 포함한 문자열(string)입니다. 실제 이미지 이름은 `${image}`을 통해 동적으로 삽입됩니다.
